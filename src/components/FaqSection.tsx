    ...
    <section id="faq" className="section-shell">
      <div className="site-container grid gap-12 lg:grid-cols-[0.72fr_1.28fr]">
        <div className="section-heading lg:sticky lg:top-28 lg:self-start">
          ...
          <a href="https://www.youtube.com/watch?v=dQw4w9WgXcQ" target="_blank" rel="noreferrer" className="secondary-button mt-7 inline-flex">Open Discord support ticket</a>
        </div>
        ...
        <div className="space-y-3">
          {FAQS.map((faq, index) => {
            ...
            return (
              <article key={faq.question} className={`surface-card overflow-hidden ${isOpen ? 'border-brand-primary/25' : ''}`}>
                <button onClick={() => setOpenIndex(isOpen ? null : index)} className="flex w-full items-center justify-between gap-6 p-5 text-left sm:p-6" aria-expanded={isOpen}>
                ...
                <div className={`grid transition-[grid-template-rows] duration-300 ${isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
                  <div className="overflow-hidden">
                    <p className="border-t border-white/8 px-5 py-5 text-sm leading-7 text-white/45 sm:px-6">{faq.answer}</p>
                  </div>
                </div>
              </article>
            );
