  ...
  return (
    <div className="preview-frame">
      ...
      <div className="relative aspect-[16/9] overflow-hidden bg-black">
        <img src={rustScientists} alt="Official Rust gameplay in an industrial interior" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="absolute left-4 top-4 z-20 hidden rounded-lg border border-white/10 bg-black/55 p-2.5 backdrop-blur-md sm:block">
          <div className="relative grid h-16 w-16 place-items-center rounded-full border border-brand-primary/35">
            ...
            <span className="h-1.5 w-1.5 rounded-full bg-brand-primary shadow-[0_0_10px_#de5b31]" />
            <span className="absolute right-3 top-5 h-1 w-1 rounded-full bg-brand-primary" />
          </div>
          <span className="mt-1.5 block text-center font-mono text-[8px] tracking-widest text-white/45">RADAR / 50M</span>
        </div>

        <EspTarget config={config} className="left-[51.3%] top-[52%] h-[20%] w-[4.6%]" name="SCI_01" distance="32M" health="84%" weapon="MP5" />
        <EspTarget config={config} className="left-[73.5%] top-[58.2%] h-[20.5%] w-[5%]" name="SCI_02" distance="41M" health="100%" weapon="LR-300" />

        {config.espSnaplines && (
          <svg className="absolute inset-0 h-full w-full" aria-hidden="true">
            <line x1="50%" y1="50%" x2="51.3%" y2="52%" stroke="#de5b31" strokeOpacity=".46" strokeDasharray="3 6" />
            <line x1="50%" y1="50%" x2="73.5%" y2="58.2%" stroke="#de5b31" strokeOpacity=".46" strokeDasharray="3 6" />
          </svg>
        )}
